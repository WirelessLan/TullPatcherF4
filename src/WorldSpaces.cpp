...
namespace WorldSpaces
{
	namespace
	{
		constexpr std::string_view kTypeName = "Worldspace";
		...
		void Prepare(const ConfigData& a_configData)
		{
			if (a_configData.Filter == FilterType::kFormID)
			{
				...
				auto& patchData = g_patchMap[worldspace];
	...
	void ReadConfigs()
	{
		g_configVec = ConfigUtils::ReadConfigs<WorldspaceParser, Parsers::Statement<ConfigData>>(kTypeName);
	}
	...
	void Patch()
	{
		...
		for (const auto& [worldspace, patchData] : g_patchMap)
		{
			if (patchData.FullName.has_value())
			{
				worldspace->fullName = patchData.FullName.value();
			}
		}
